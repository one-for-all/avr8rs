...
pub fn avr_instruction(cpu: &mut CPU) {
    ...
    // println!("ins: {:?}, opcode: 0x{:x}", instruction, opcode);
    ...
    match instruction {
        instructions::Instruction::ADC => {
            ...
            let sum = d as u16 + r as u16 + (cpu.data[95] & 1) as u16;
            let R = (sum & 255) as u8;
        ...
        instructions::Instruction::ADIW => {
            ...
            let R = (value + ((opcode & 0xf) | ((opcode & 0xc0) >> 2))) & 0xffff;
        ...
        instructions::Instruction::CP => {
            ...
            let val2 = cpu.get_data((opcode & 0xf) | ((opcode & 0x200) >> 5));
        ...
        instructions::Instruction::CPC => {
            ...
            let arg1 = cpu.get_data((opcode & 0x1f0) >> 4);
            let arg2 = cpu.get_data((opcode & 0xf) | ((opcode & 0x200) >> 5));
            let mut sreg = cpu.data[95];
            let r = arg1.wrapping_sub(arg2).wrapping_sub(sreg & 1);
            sreg = (sreg & 0xc0)
                | (if !r != 0 && (sreg >> 1) & 1 != 0 {
                    2
                } else {
        ...
        instructions::Instruction::CPI => {
            ...
            let arg1 = cpu.get_data(((opcode & 0xf0) >> 4) + 16);
            let arg2 = ((opcode & 0xf) | ((opcode & 0xf00) >> 4)) as u8;
            let r = arg1.wrapping_sub(arg2);
            let mut sreg = cpu.data[95] & 0xc0;
            sreg |= ternary!(r, 0, 2);
            sreg |= ternary!(128 & r, 4, 0);
            sreg |= ternary!((arg1 ^ arg2) & (arg1 ^ r) & 128, 8, 0);
        ...
        instructions::Instruction::SBC => {
            ...
            sreg = (sreg & 0xc0)
                ...
                | (if val2.wrapping_add(sreg & 1) > val1 {
                    1
                } else {
                    0
                });
        ...
        instructions::Instruction::SBCI => {
            ...
            let val1 = cpu.get_data(((opcode & 0xf0) >> 4) + 16);
            let val2 = (opcode & 0xf) as u8 | ((opcode & 0xf00) >> 4) as u8;
            let mut sreg = cpu.data[95];
            let R = val1.wrapping_sub(val2).wrapping_sub(sreg & 1);
            cpu.set_data(((opcode & 0xf0) >> 4) + 16, R);
            sreg = (sreg & 0xc0)
                | (if R == 0 && (sreg >> 1) & 1 != 0 { 2 } else { 0 })
                | (if val2 as u16 + (sreg & 1) as u16 > val1 as u16 {
                    1
                } else {
                    0
                });
        ...
        instructions::Instruction::SBIW => {
            ...
            let a = cpu.get_data_u16(i);
            let l = (opcode & 0xf) | ((opcode & 0xc0) >> 2);
            let R = a.wrapping_sub(l);
            cpu.set_data_u16(i, R);
            let mut sreg = cpu.data[95] & 0xc0;
            sreg |= ternary!(R, 0, 2);
            sreg |= ternary!(0x8000 & R, 4, 0);
        ...
        instructions::Instruction::STX => {
            ...
            cpu.write_data(cpu.get_data_u16(26), cpu.get_data((opcode & 0x1f0) >> 4));
            cpu.cycles += 1;
        }
